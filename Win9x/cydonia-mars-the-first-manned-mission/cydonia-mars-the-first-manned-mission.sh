...
#Game        : Cydonia Mars - The First Manned Mission
#
#Source      : Archive
#
#Runner      : DosBox
#
#Description : This script will extract the E archive into ROMs folder .
#              1: Get Game file
#              2: Create Game folder in ROMS/windows9x add conf extention to folder name (ESDE needs this)
#              3: Extract Game Archive 

#Constants
ROMs_FOLDER="$HOME/Games/ROMs/windows9x"
CONF_NAME="cydonia-mars-the-first-manned-mission"
CONF_FILE_NAME="$CONF_NAME.conf"
CONF_FILE_URL="https://raw.githubusercontent.com/appoloin/bash-scripts/refs/heads/main/Win9x/$CONF_NAME/$CONF_FILE_NAME"
FILE_FILTER=""
...
select_archive() {
    ...
    FILE=$(zenity --file-selection \
                  ...
                  --file-filter="Archive | *.zip *.7z *.rar *.7z.001 *.001 *.zip.001 *.r00 *.tar.gz")
...
main(){
    ...
    mkdir -p "$ROMs_FOLDER/$CONF_FILE_NAME"
    extract_archive "$FILES" "$ROMs_FOLDER/$CONF_FILE_NAME" "e" "$FILE_FILTER"
    if [ $? -ne 0 ]; then 
