...
ENGINE_NAME="org.yamagi.YamagiQ2"
ENGINE_DATA="$HOME/.var/app/org.yamagi.YamagiQ2/.yq2"
FILE_FILTER=""
PATCH_URL="https://www.dropbox.com/scl/fi/bvzr92j1x8zp84hmp32dh/q2-3.20-x86-full-ctf.7z.001?rlkey=joqrsrewpnxhlslevtnj694mx&st=wvxuw2xy&dl=1"
PATCH_NAME="q2-3.20-x86-full-ctf.7z.001"
...
main(){
    ...
    download_file "$PATCH_URL" "$TEMP_FOLDER" "$PATCH_NAME" 
    #extract to GameFolder/temp  
    extract_archive "$TEMP_FOLDER/$PATCH_NAME" "$ROMs_FOLDER/$GAME_NAME" "x"
    if [ $? -ne 0 ]; then
        zenity --notification --text="Error Patch extract failed" --title="Game Install" 
        rm -f -r "$ROMs_FOLDER/$GAME_NAME"
        exit 1
    fi
    ...
    find "$TEMP_FOLDER/app/baseq2" -type f -iname "Pak0.pak" -exec mv {} "$ROMs_FOLDER/$GAME_NAME/baseq2" \;
    find "$TEMP_FOLDER/app/baseq2"  -mindepth 1 -maxdepth 1 -iname "video"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/baseq2" \;
    find "$TEMP_FOLDER/app/"  -mindepth 1 -maxdepth 1 -iname "music"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;

    find "$TEMP_FOLDER/app/xatrix"  -mindepth 1 -maxdepth 1 -name "*.pak"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/xatrix" \;
    find "$TEMP_FOLDER/app/xatrix"  -mindepth 1 -maxdepth 1 -name "video"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/xatrix" \;

    find "$TEMP_FOLDER/app/rogue"  -mindepth 1 -maxdepth 1 -name "*.pak"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/rogue" \;
    find "$TEMP_FOLDER/app/rogue"  -mindepth 1 -maxdepth 1 -name "video"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/rogue" \;
    ...
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/baseq2" "$ENGINE_DATA/baseq2"   # Link the folders 
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/music" "$ENGINE_DATA/music"   
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/xatrix" "$ENGINE_DATA/xatrix"   
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/rogue" "$ENGINE_DATA/rogue"   


    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"

    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\nflatpak run $ENGINE_NAME -datadir $ROMs_FOLDER/$GAME_NAME" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
