...
ENGINE_NAME="tk.deat.Jazz2Resurrection"
ENGINE_DATA="$HOME/.var/app/tk.deat.Jazz2Resurrection/data/Jazz² Resurrection"
FILE_FILTER=""
...
main(){
    ...
    rm -Rf "$ENGINE_DATA/Source"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/" "$ENGINE_DATA/Source"   # Link the folders 
