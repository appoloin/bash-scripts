...
ENGINE_NAME="com.corsixth.corsixth"
ENGINE_DATA="$HOME/.var/app/com.corsixth.corsixth/config/CorsixTH"
FILE_FILTER=""
...
main(){
    ...
    #Create config file
    rm -Rf "$ENGINE_DATA/config.txt"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to write to
    echo -e "fullscreen = true\nui_scale = 2\n\theme_hospital_install = [[$ROMs_FOLDER/$GAME_NAME]]" > "$ENGINE_DATA/config.txt"
