...
#Game        : Medal of Honor - Allied Assult War Chest
#
#Source      : Archive GOG
#
#Runner      : Linux Port
...
ENGINE_URL="https://www.dropbox.com/scl/fi/o1jun9n6c5pm41xfa4ig7/openmohaa-v0.82.1-linux-amd64.zip?rlkey=o2v8j7jgqercntmkef9fqev7f&st=5nij9quz&dl=1"
ENGINE_NAME="aopenmohaa-v0.82.1-linux-amd64.zip"
ENGINE_EXE=""
...
main(){
    ...
    chmod +x "$ROMs_FOLDER/$GAME_NAME/launch_openmohaa_base"    
    chmod +x "$ROMs_FOLDER/$GAME_NAME/launch_openmohaa_spearhead"    
    chmod +x "$ROMs_FOLDER/$GAME_NAME/launch_openmohaa_breakthrough"    
    chmod +x "$ROMs_FOLDER/$GAME_NAME/omohaaded"    
    chmod +x "$ROMs_FOLDER/$GAME_NAME/openmohaa"    
    ...
    find "$TEMP_FOLDER/app"  -mindepth 1 -maxdepth 1 -name "main"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER/app"  -mindepth 1 -maxdepth 1 -name "mainta"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER/app"  -mindepth 1 -maxdepth 1 -name "maintt"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;

    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"
    
    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\n$ROMs_FOLDER/$GAME_NAME/launch_openmohaa_base +set fs_homepath $ROMs_FOLDER/$GAME_NAME +set r_mode -1 +set r_customheight 1060 +set r_customwidth 1920" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
    echo -e "#!/bin/bash\n\n$ROMs_FOLDER/$GAME_NAME/launch_openmohaa_spearhead +set fs_homepath $ROMs_FOLDER/$GAME_NAME +set r_mode -1 +set r_customheight 1060 +set r_customwidth 1920" > "$ROMs_FOLDER/spearhead.sh"
    echo -e "#!/bin/bash\n\n$ROMs_FOLDER/$GAME_NAME/launch_openmohaa_breakthrough +set fs_homepath $ROMs_FOLDER/$GAME_NAME +set r_mode -1 +set r_customheight 1060 +set r_customwidth 1920" > "$ROMs_FOLDER/breakthrough.sh"
