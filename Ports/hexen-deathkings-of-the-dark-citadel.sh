...
ENGINE_NAME="org.zdoom.UZDoom"
FILE_FILTER=""
MUSIC_URL="https://www.dropbox.com/scl/fi/gu1cz0pduyj4hn32z8wca/HEXEN64.7z.001?rlkey=bcwndbrojxac9tlc348eum4b4&st=a4fd42op&dl=1"
MUSIC_NAME="HEXEN64.7z.001"
...
main(){
    ...
    find "$TEMP_FOLDER"  -mindepth 1 -maxdepth 1 -name "MUSIC"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER"  -mindepth 1 -maxdepth 1 -name "HEXDD.WAD"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER"  -mindepth 1 -maxdepth 1 -name "HEXEN.WAD"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    ...
    echo -e "[IWADSearch.Directories]\n\nPath=$ROMs_FOLDER/$GAME_NAME\n\nPath=\$DOOMWADDIR" > "$ROMs_FOLDER/$GAME_NAME/uzdoom.ini"


    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"
    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\nflatpak run $ENGINE_NAME \
                                   -config $ROMs_FOLDER/$GAME_NAME/uzdoom.ini \
                                   -file HEXEN.WAD \
                                   -savedir $ROMs_FOLDER/$GAME_NAME/saves" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
