...
ENGINE_NAME="io.github.dxx_rebirth.dxx-rebirth"
ENGINE_DATA="$HOME/.var/app/io.github.dxx_rebirth.dxx-rebirth/data/.d2x-rebirth/"
FILE_FILTER=""
MUSIC_URL="https://www.dropbox.com/scl/fi/w7f41nvfsxhal3trxkp7f/d2x-rebirth-addons.7z.001?rlkey=9ujuc3d2uepzdj4tti4v6rxjs&st=ouk38w8j&dl=1"
MUSIC_NAME="d2x-rebirth addons.7z.001"
...
main(){
    ...
    find "$ROMs_FOLDER/$GAME_NAME/app/Descent 2"  -mindepth 1 -maxdepth 1 -name "*"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$ROMs_FOLDER/$GAME_NAME/__support/save"  -mindepth 1 -maxdepth 1 -name "*"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;


    zenity --notification --text="Downloading Music" --title="Game Install" 
    #get music from dropbox
    download_file "$MUSIC_URL" "$TEMP_FOLDER" "$MUSIC_NAME" 
    #extract to GameFolder/temp  
    extract_archive "$TEMP_FOLDER/$MUSIC_NAME" "$ROMs_FOLDER/$GAME_NAME" "e"
    if [ $? -ne 0 ]; then
    ...
    rm -Rf "$ENGINE_DATA/data"    #remove old game files 
    ...
    echo -e "#!/bin/bash\n\nflatpak run $ENGINE_NAME d2x" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
