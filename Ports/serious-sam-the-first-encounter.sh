...
main(){
    ...
    rm -Rf "$ENGINE_DATA/serioussam"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/" "$ENGINE_DATA/serioussam"   # Link the folders 


    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"

    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\nflatpak run $ENGINE_NAME --command=serioussam" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
