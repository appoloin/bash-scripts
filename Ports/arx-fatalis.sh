...
ENGINE_URL="https://www.dropbox.com/scl/fi/un2kciilonxbwq051wilm/arx-libertatis-1.2.1-linux.tar.xz?rlkey=k18oth4drdjuj9co3wlemj5j6&st=osf57p50&dl=1"
ENGINE_NAME="arx-libertatis-1.2.1-linux.tar.xz"
ENGINE_NAME_2="arx-libertatis-1.2.1-linux.tar"
ENGINE_NAME_3="arx-libertatis-1.2.1-linux"
ENGINE_EXE="arx"
...
main(){
    ...
    extract_archive "$TEMP_FOLDER/$ENGINE_NAME_2" "$TEMP_FOLDER" "x" "$FILE_FILTER"
    if [ $? -ne 0 ]; then 
        #remove Game folder
        rm -f -r "$ROMs_FOLDER/$GAME_NAME"
        exit 1
    fi
    #Move Engine out of temp folder
    find "$TEMP_FOLDER/$ENGINE_NAME_3"  -mindepth 1 -maxdepth 1 -name "*"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    ...
    #Install game into engine
    "$ROMs_FOLDER/$GAME_NAME/bin/arx-install-data"  --batch --source "$TEMP_FOLDER/app" --data-dir "$ROMs_FOLDER/$GAME_NAME/data"
    if [ $? -ne 0 ]; then
        echo "Game Data install failed"
        zenity --error --text="Error: Engine failed on data install."
      #  rm -f -r "$ROMs_FOLDER/$GAME_NAME"
        exit 1
    fi
