...
main(){
    ...
    find "$TEMP_FOLDER/app/DATA" -mindepth 1 -maxdepth 1 -iname "main.sfx"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/games/tr3" \;
    mkdir -p "$ROMs_FOLDER/$GAME_NAME/games/tr3/levels"
    find "$TEMP_FOLDER/app/DATA" -mindepth 1 -maxdepth 1 -iname "*.tr2"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/games/tr3/levels" \;
    mkdir -p "$ROMs_FOLDER/$GAME_NAME/games/tr3/fmv"
    find "$TEMP_FOLDER/app/fmv"  -mindepth 1 -maxdepth 1 -iname "*"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/games/tr3/fmv" \;
    mkdir -p "$ROMs_FOLDER/$GAME_NAME/games/tr3/cuts"
    find "$TEMP_FOLDER/app/cuts"  -mindepth 1 -maxdepth 1 -iname "*.tr2"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/games/tr3/cuts" \;
    mkdir -p "$ROMs_FOLDER/$GAME_NAME/games/tr3/audio"
    find "$TEMP_FOLDER/app/audio"  -mindepth 1 -maxdepth 1 -iname "cdaudio.wad"  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/games/tr3/audio" \;



    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"
    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\n$ROMs_FOLDER/$GAME_NAME/$ENGINE_EXE --mod tr3" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
