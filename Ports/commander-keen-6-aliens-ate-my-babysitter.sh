...
main(){
    ...
    rm -Rf "$ENGINE_DATA/CKeen6"    #remove old game files 
    rm -Rf "$ENGINE_DATA/KEEN6"    
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/CKeen6" "$ENGINE_DATA/KEEN6"   # Link the folders 
