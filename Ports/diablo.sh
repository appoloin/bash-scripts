...
ENGINE_NAME="org.diasurgical.DevilutionX"
ENGINE_DATA="$HOME/.var/app/org.diasurgical.DevilutionX/data/diasurgical/"
FILE_FILTER=""
...
main(){
    ...
    rm -Rf "$ENGINE_DATA/devilution"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/" "$ENGINE_DATA/devilution"   # Link the folders 
