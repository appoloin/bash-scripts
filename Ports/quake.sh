...
ENGINE_NAME="net.sourceforge.quakespasm.Quakespasm"
ENGINE_DATA="$HOME/.var/app/net.sourceforge.quakespasm.Quakespasm/"
FILE_FILTER=""
MUSIC_ID1_URL="https://www.dropbox.com/scl/fi/fdgxa4dus3hp1xhbt80ri/id1.7z.001?rlkey=9c86oaphmq2sj0xbgjfedlitz&st=l6momo1v&dl=1"
MUSIC_ID1_NAME="id1.7z.001"
MUSIC_HIP_URL="https://www.dropbox.com/scl/fi/r8hca2f2jwex0rzvdyuh4/hipnotic.7z.001?rlkey=ym5kmn4qhnbohksou4j96hmuc&st=1bxxwinj&dl=1"
MUSIC_HIP_NAME="hipnotic.7z.001"
MUSIC_ROG_URL="https://www.dropbox.com/scl/fi/7ulo0nc34cofvslsbdzmx/rogue.7z.001?rlkey=hmrtod0h2h97raw0ubuduxawd&st=vzczos8j&dl=1"
MUSIC_ROG_NAME="rogue.7z.001"
...
main(){
    ...
    zenity --notification --text="Downloading and Installing Patch" --title="Game Install" 
    #get music from dropbox
    mkdir -p "$ROMs_FOLDER/$GAME_NAME/id1/music" 
    download_file "$MUSIC_ID1_URL" "$TEMP_FOLDER" "$MUSIC_ID1_NAME" 
    #extract to GameFolder/temp  
    extract_archive "$TEMP_FOLDER/$MUSIC_ID1_NAME" "$ROMs_FOLDER/$GAME_NAME/id1/music" "e"
    if [ $? -ne 0 ]; then
        zenity --notification --text="Error Music extract failed" --title="Game Install" 
    fi

    mkdir -p "$ROMs_FOLDER/$GAME_NAME/rogue/music" 
    download_file "$MUSIC_ROG_URL" "$TEMP_FOLDER" "$MUSIC_ROG_NAME" 
    #extract to GameFolder/temp  
    extract_archive "$TEMP_FOLDER/$MUSIC_ROG_NAME" "$ROMs_FOLDER/$GAME_NAME/rogue/music" "e"
    if [ $? -ne 0 ]; then
        zenity --notification --text="Error Music extract failed" --title="Game Install" 
    fi

    mkdir -p "$ROMs_FOLDER/$GAME_NAME/hipnotic/music" 
    download_file "$MUSIC_HIP_URL" "$TEMP_FOLDER" "$MUSIC_HIP_NAME" 
    #extract to GameFolder/temp  
    extract_archive "$TEMP_FOLDER/$MUSIC_ROG_NAME" "$ROMs_FOLDER/$GAME_NAME/hipnotic/music" "e"
    if [ $? -ne 0 ]; then
        zenity --notification --text="Error Music extract failed" --title="Game Install" 
    fi
    ...
    find "$TEMP_FOLDER/app/id1" -type f -iname "*.pak" -exec mv {} "$ROMs_FOLDER/$GAME_NAME/id1" \;
    find "$ROMs_FOLDER/$GAME_NAME/id1/PAK0.PAK" -type f -iname "*.pak" -exec mv {} "$ROMs_FOLDER/$GAME_NAME/id1/pak0.pak" \;
    find "$ROMs_FOLDER/$GAME_NAME/id1/PAK1.PAK" -type f -iname "*.pak" -exec mv {} "$ROMs_FOLDER/$GAME_NAME/id1/pak1.pak" \;


    find "$TEMP_FOLDER/app/hipnotic" -type f -iname "*.pak" -exec mv {} "$ROMs_FOLDER/$GAME_NAME/hipnotic" \;
    find "$TEMP_FOLDER/app/rogue" -type f -iname "*.pak" -exec mv {} "$ROMs_FOLDER/$GAME_NAME/rogue" \;


    # Link flatpak data folder to GAME_NAME, Engine can now see games 
    rm -Rf "$ENGINE_DATA"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME" "$ENGINE_DATA/data"   # Link the folders 


    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"

    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\nflatpak run $ENGINE_NAME -fullscreen" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
