...
ENGINE_NAME="io.github.maddecoder.Classic-RBDOOM-3-BFG"
ENGINE_DATA="$HOME/.var/app/io.github.maddecoder.Classic-RBDOOM-3-BFG/data/.doombfa/"
FILE_FILTER=""
...
main(){
    ...
    rm -Rf "$ENGINE_DATA/base"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/" "$ENGINE_DATA/base"   # Link the folders 

    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"

    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\nflatpak run $ENGINE_NAME +set fs_savepath $ROMs_FOLDER/$GAME_NAME \
                                                +set fs_configpath $ROMs_FOLDER/$GAME_NAME" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
