...
OTHER_GAME_NAME="rollercoaster-tycoon-1"
GAME_NAME="$NAME.sh"
...
ENGINE_NAME="io.openrct2.OpenRCT2"
FILE_FILTER=""
...
main(){
    ...
    #setting data path
    flatpak run io.openrct2.OpenRCT2 set-rct2 "$ROMs_FOLDER/$GAME_NAME"
    ...
    if zenity --question --text="Install Rollercoster Tycoon 1 from Archive"; then
        echo "Yes"
        zenity --notification --text="Installing Rollercoster Tycoon 1" --title="Game Install"
        select_archive  #Get Archive location
        if [ $? -eq 0 ]; then
            mkdir -p "$TEMP_FOLDER/RCT1"
            extract_archive "$FILES" "$TEMP_FOLDER/RCT1" "e"
            if [ $? -eq 0 ]; then 
                EXE_PATH=$(find "$TEMP_FOLDER/RCT1" -type f -iname "*.exe" | head -n 1)

                "$TEMP_FOLDER/$INNO_EXE" -d "$ROMs_FOLDER/$OTHER_GAME_NAME" "$EXE_PATH"
                if [ $? -eq 0 ]; then
                    find "$ROMs_FOLDER/$OTHER_GAME_NAME/app"  -mindepth 1 -maxdepth 1 -name "*"  -exec mv {} "$ROMs_FOLDER/$OTHER_GAME_NAME" \;
                    find "$ROMs_FOLDER/$OTHER_GAME_NAME/__support/save"  -mindepth 1 -maxdepth 1 -name "*"  -exec mv {} "$ROMs_FOLDER/$OTHER_GAME_NAME" \;
                    touch "$ROMs_FOLDER/$OTHER_GAME_NAME/noload.txt"
                else
                    echo "Inno failed to extract RCT1: '$EXE_PATH'"
                fi
            else
                echo "Error extractin File RCT1" 
            fi
        else
            echo "Error Selecting File RCT1" 
        fi
    fi
