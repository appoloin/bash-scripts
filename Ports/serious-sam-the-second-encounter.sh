...
ENGINE_NAME="io.itch.tx00100xt.SeriousSamClassic"
ENGINE_DATA="$HOME/.var/app/io.itch.tx00100xt.SeriousSamClassic/data/Serious-Engine/"
FILE_FILTER=""
...
main(){
    ...
    rm -Rf "$ENGINE_DATA/serioussamse"    #remove old game files 
    mkdir -p "$ENGINE_DATA"   #make sure there is folder to link with
    ln -s -f -n "$ROMs_FOLDER/$GAME_NAME/" "$ENGINE_DATA/serioussamse"   # Link the folders 


    touch "$ROMs_FOLDER/$GAME_NAME/noload.txt"

    #Create ES_DE launch file with engine code
    echo -e "#!/bin/bash\n\nflatpak run --command=serioussamse $ENGINE_NAME" > "$ROMs_FOLDER/$GAME_NAME/$GAME_NAME"
