...
ENGINE="https://www.dropbox.com/scl/fi/blc0wvv7tdzo5161wu20q/W.C.A.T.-Beta-0.8-R4-DOS.zip?rlkey=biy9fq3v9l7ow0c3709kfej5n&st=8mvn9t1x&dl=1"
ENGINE_NAME="W.C.A.T. - Beta 0.8 R4 (DOS).zip"
...
main(){
    ...
    zenity --notification --text="Downloading Engine MOD" --title="Game Install"
    #get ENGINE archive from dropbox
    download_file "$ENGINE" "$TEMP_FOLDER" "$ENGINE_NAME" 
    # Check if wget succeeded
    if [ $? -ne 0 ]; then
        rm -f -r "$ROMs_FOLDER/$CONF_FILE_NAME"
        exit 1
    ...
    #extract to ROMs/pc/Game folder/temp  
    extract_archive "$TEMP_FOLDER/$ENGINE_NAME" "$ROMs_FOLDER/$CONF_FILE_NAME" "x"
    if [ $? -ne 0 ]; then
        zenity --error --text="Error: Innoextract extract failed."
        rm -f -r "$ROMs_FOLDER/$CONF_FILE_NAME"
        exit 1
