...
CONF_FILE_URL="https://raw.githubusercontent.com/appoloin/bash-scripts/refs/heads/main/DOS/mission-critical/mission-critical.conf"
CONF_FILE_NAME="mission-critical.conf"
...
main(){
    ...
    zenity --notification --text="Extracting archive" --title="Game Install"
    #extract to ROMs/dos/Game folder/temp  
    extract_archive "$FILES" "$TEMP_FOLDER" "x"
    if [ $? -ne 0 ]; then
        zenity --error --text="Error: archive extract failed."
    ...
    find "$ROMs_FOLDER/$CONF_FILE_NAME/__support/save" -mindepth 1 -maxdepth 1 -name "*"  -exec cp {} -r "$ROMs_FOLDER/$CONF_FILE_NAME" \;
