...
CONF_FILE_URL="https://raw.githubusercontent.com/appoloin/bash-scripts/refs/heads/main/DOS/alone-in-the-dark-2/alone-in-the-dark-2.conf"
CONF_FILE_NAME="alone-in-the-dark-2.conf"
CONF_FILE_SETUP_URL="https://raw.githubusercontent.com/appoloin/bash-scripts/refs/heads/main/DOS/alone-in-the-dark-2/alone-in-the-dark-2-setup.conf"
CONF_FILE_SETUP_NAME="alone-in-the-dark-2-setup.conf"
...
TEMP_FOLDER="$HOME/Games/ROMs/dos/alone-in-the-dark-2.conf/temp"
...
main(){
    ...
    #Move files/folders from installer sub folder to main game folder
    find "$ROMs_FOLDER/$CONF_FILE_NAME/__support/app" -mindepth 1 -maxdepth 1 -name "*"  -exec cp {} -r "$ROMs_FOLDER/$CONF_FILE_NAME" \;
    ...
    mv "$ROMs_FOLDER/$CONF_FILE_NAME/INDARK2/INDARK2.CFG" "$ROMs_FOLDER/$CONF_FILE_NAME/INDARK2/INDARK2.CFG.old"

    #Download setup conf file from github
    download_file "$CONF_FILE_SETUP_URL"  "$TEMP_FOLDER" "$CONF_FILE_SETUP_NAME"
    # Check if wget succeeded
    if [ $? -ne 0 ]; then
        echo "Failed to download: '$CONF_FILE_SETUP_URL'"
        zenity --error --text="Error: Conf download failed \n$CONF_FILE_SETUP_URL"
        rm -f -r $ROMs_FOLDER/$CONF_FILE_NAME
        exit 1
    fi
    ...
    #Run Config
    zenity --notification --text="Use Setup to set Audio. Remmeber to press "Update"" --title="Game Install"
    flatpak run io.github.dosbox-staging --working-dir "$ROMs_FOLDER/$CONF_FILE_NAME" -conf "$TEMP_FOLDER/$CONF_FILE_SETUP_NAME"
    ...
    find "$ROMs_FOLDER/$CONF_FILE_NAME" -type f \( -iname "goggame*" \) -exec rm {} \;
