...
FILE_FILTER="-ir!*.BMV -ir!*.CDP -ir!*.MUS -ir!*.SCN -ir!GDATA -ir!HOPPER -ir!INDEX -ir!SAMPLE.BNK -ir!*.SMP -ir!*.TXT -ir!*.UDX"
...
main(){
    ...
    for item in "${ISOs[@]}"; do
        ...
        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "ENGLISH.SMP" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME/ENGLISH$DISC_NUMBER.SMP" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "ENGLISH.TXT" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME/ENGLISH$DISC_NUMBER.TXT" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "ENGLISH.UDX" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME/ENGLISH$DISC_NUMBER.IDX" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "US.SMP" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/US$DISC_NUMBER.SMP" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "US.TXT" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/US$DISC_NUMBER.TXT" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "US.IDX" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/US$DISC_NUMBER.IDX" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "FRENCH.SMP" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/FRENCH$DISC_NUMBER.SMP" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "FRENCH.TXT" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/FRENCH$DISC_NUMBER.TXT" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "FRENCH.IDX" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/FRENCH$DISC_NUMBER.IDX" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "GERMAN.SMP" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/GERMAN$DISC_NUMBER.SMP" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "GERMAN.TXT" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/GERMAN$DISC_NUMBER.TXT" \;

        find "$ROMs_FOLDER/$GAME_NAME" -type f \( -iname "GERMAN.IDX" \)  -exec mv {} "$ROMs_FOLDER/$GAME_NAME/GERMAN$DISC_NUMBER.IDX" \;
