...
main(){
    ...
    extract_archive "$FILES" "$TEMP_FOLDER" "x" "$FILE_FILTER"
    if [ $? -ne 0 ]; then 
        #remove Game folder
    ...
    find "$TEMP_FOLDER" -type d \( -iname "GLOBAL" -o \
                                   -iname "INFACE" -o \
                                   -iname "INTRO" -o \
                                   -iname "??"  \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;

    mkdir -p "$ROMs_FOLDER/$GAME_NAME/SUPPORT"
    find "$TEMP_FOLDER/SUPPORT" -type f \( -iname "assets.z" -o \
                                           -iname "pvteye.z"  \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME/SUPPORT" \;
