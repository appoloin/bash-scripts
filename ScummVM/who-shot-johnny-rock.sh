...
main(){
    ...
    mkdir "$ROMs_FOLDER/$GAME_NAME/$TEMP_FOLDER"
        
    zenity --notification --text="Extracting files from iso" --title="Game Install"
    extract_archive "$EXE_PATH" "$ROMs_FOLDER/$GAME_NAME" "x" "$FILE_FILTER"
    #move game install folder to es-de pc folder

    if [ $? -ne 0 ]; then 
        #remove Game folder
        zenity --notification --text="Error extracting files from iso" --title="Game Install"
