...
#Game        : Nightlong - Union City Conspiracy
#
#Source      : Archive ISO
#
#Runner      : Scummvm
...
main(){
    ...
    find "$TEMP_FOLDER" -type f \( -iname "DATA.NL" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER" -type d \( -iname "FMV" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER" -type d \( -iname "DATA" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
