...
main(){
    ...
    find "$TEMP_FOLDER" -type d \( -iname video -o \
                                   -iname data  \) -exec cp {} -r "$ROMs_FOLDER/$GAME_NAME" \;
    
    find "$TEMP_FOLDER" -type f \( -iname "ft.la*" -o \
                                                 -iname "monster.sou"  \) -exec cp {} "$ROMs_FOLDER/$GAME_NAME" \;
