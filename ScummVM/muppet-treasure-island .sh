...
main(){
    ...
    for item in "${ISOs[@]}"; do
        echo "Selected: $item"
        extract_archive "$item" "$TEMP_FOLDER" "x" "$FILE_FILTER"
        if [ $? -ne 0 ]; then 
            #remove Game folder
            rm -f -r "$ROMs_FOLDER/$GAME_NAME"
            exit 1
        ...
        find "$TEMP_FOLDER" -type f \( -iname "MTI1.MPL" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
        find "$TEMP_FOLDER" -type f \( -iname "MTI2.MPX" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
        find "$TEMP_FOLDER" -type f \( -iname "MTI3.MPX" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
        find "$TEMP_FOLDER" -type f \( -iname "MTI4.MPX" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;

        if [ $DISC_NUMBER = 1 ]; then 
            echo "Moving: MTPLAY32 from disc 1"

            find "$TEMP_FOLDER" -type d \( -iname "MTPLAY32" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME/MTPLAY32" \;
        fi
        
        if [ $DISC_NUMBER = 3 ]; then 
            echo "Moving: VIDEO from disc 3"

            find "$TEMP_FOLDER" -type d \( -iname "VIDEO" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME/VIDEO" \;
        fi
