...
main(){
    ...
    "$TEMP_FOLDER/$INNO_EXE" --include "app/Lands of Lore 1" -d "$TEMP_FOLDER" "$EXE_PATH"
    if [ $? -ne 0 ]; then
    ...
    find "$TEMP_FOLDER/app/Lands of Lore 1" -maxdepth 1 \(  -iname "*.PAK" -o \
                                                            ...
                                                            -iname "*.TLK" -o \
                                                            -iname "WESTWOOD.*" -o \
                                                            -iname "FILEDATA.FDT" \) -exec cp {} -r "$ROMs_FOLDER/$GAME_NAME" \;

    ISO_PATH=$(find "$TEMP_FOLDER/app/Lands of Lore 1" -type f -iname "GAME.DAT" | head -n 1)


    zenity --notification --text="Extracting files from iso" --title="Game Install"
    extract_archive "$ISO_PATH" "$ROMs_FOLDER/$GAME_NAME" "x" "$FILE_FILTER"
    if [ $? -ne 0 ]; then 
        #remove Game folder
        rm -f -r "$ROMs_FOLDER/$GAME_NAME"
        exit 1
