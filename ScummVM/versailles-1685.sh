...
#Game        : Versailles 1685
#
#Source      : Archive ISO*2
#
#Runner      : Scummvm
#
...
main(){
    ...
    find "$TEMP_FOLDER/DATAS_V" -mindepth 1 -maxdepth 1  \( -iname "*" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER/INSTALL/DATA"  -mindepth 1 -maxdepth 1 \( -iname "*" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
    find "$TEMP_FOLDER/INSTALL/WIN" -type f \( -iname "PROGRAM.Z" \) -exec mv {} "$ROMs_FOLDER/$GAME_NAME" \;
