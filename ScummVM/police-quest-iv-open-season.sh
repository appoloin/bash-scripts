...
#Game        : Police Quest 4 - Open Season
#
...
main(){
    ...
    ISO=$(find "$TEMP_FOLDER" -type f -iname "*.gog" | head -n 1)
    extract_archive "$ISO" "$ROMs_FOLDER/$GAME_NAME" "x"
    if [ $? -ne 0 ]; then
        echo "Failed to extract ISO: '$ISO'"
        zenity --error --text="Error: Extraction of game ISO failed \n'$$ISO'."
        rm -f -r "$ROMs_FOLDER/$GAME_NAME"
        exit 1
    fi

    find "$TEMP_FOLDER" -type f \( -iname "Manual.pdf" \) -exec cp {} "$ROMs_FOLDER/$GAME_NAME" \;
